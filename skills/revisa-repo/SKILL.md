...
> **Nota:** esta pasta é um espelho, para navegação no repositório, da skill ativa que o Claude Code de fato carrega em [`.claude/skills/revisa-repo/`](../../.claude/skills/revisa-repo/). Para alterar o comportamento da skill, edite a versão em `.claude/skills/` e replique a mudança aqui — editar só esta cópia não muda o que roda nas conversas.
